...
use crate::{menus::Menu, my_app::AppState, theme::prelude::*};
...
fn go_back_on_click(
    _: On<Pointer<Click>>,
    screen: Res<State<AppState>>,
    mut next_menu: ResMut<NextState<Menu>>,
) {
    next_menu.set(if screen.get() == &AppState::Title {
        Menu::Main
    } else {
        Menu::Pause
    });
...
fn go_back(screen: Res<State<AppState>>, mut next_menu: ResMut<NextState<Menu>>) {
    next_menu.set(if screen.get() == &AppState::Title {
        Menu::Main
    } else {
        Menu::Pause
